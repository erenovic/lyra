...
def get_cpp_flags() -> list[str]:
    # -g0 overrides Python's default -g. On aarch64, the combination of -g and
    # heavy PyTorch template expansion (scatter_ext) makes GCC emit broken
    # DWARF/CFI directives that the assembler rejects.
    return ["-O3", "-DWITH_CUDA", "-g0"] + _additional_include_flags()
